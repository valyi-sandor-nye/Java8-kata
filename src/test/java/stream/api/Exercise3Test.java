...
public class Exercise3Test extends ClassicOnlineStore {
    ...
    public void howManyItemsWanted() {
         ...
         */
        long sum = customerList.stream().flatMap(cust -> cust.getWantToBuy().stream()).count();
    ...
    public void richestCustomer() {
        ...
        Optional<Integer> richestCustomer = customerList.stream().map(cust -> cust.getBudget()).max(comparator);
    ...
    public void youngestCustomer() {
         ...
         * Don't use {@link Stream#sorted}
         */
        Comparator<Customer> comparator = ((cust1, cust2) ->
                cust1.getAge().compareTo(cust2.getAge()));
